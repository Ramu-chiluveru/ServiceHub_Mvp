        ...
        <div className="w-full lg:w-1/4 flex items-center gap-2">
          <div className="relative">
                ...
                >
                  ...
                  Sort by: {sortBy} {sortBy === 'asc' ? <ArrowUp size={14} className="text-indigo-600" /> : <ArrowDown size={14} className="text-indigo-600" />}
                </button>
                {showFilterDropdown && (
                  <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-xl shadow-lg z-10 overflow-hidden">
                    <button onClick={() => handleSortChange('date')} className="w-full px-4 py-3 text-left text-sm hover:bg-indigo-50 flex items-center justify-between transition-colors">
                      <span className="flex items-center"><Star size={16} className="mr-2 text-indigo-600" /> Date</span>
                      {sortBy === 'date' && (sortBy === 'asc' ? <ArrowUp size={14} className="text-indigo-600" /> : <ArrowDown size={14} className="text-indigo-600" />)}
                    </button>
                    <button onClick={() => handleSortChange('price')} className="w-full px-4 py-3 text-left text-sm hover:bg-indigo-50 flex items-center justify-between transition-colors">
                      <span className="flex items-center"><DollarSign size={16} className="mr-2 text-indigo-600" /> Price</span>
                      {sortBy === 'price' && (sortBy === 'asc' ? <ArrowUp size={14} className="text-indigo-600" /> : <ArrowDown size={14} className="text-indigo-600" />)}
                    </button>
                    <button onClick={() => handleSortChange('status')} className="w-full px-4 py-3 text-left text-sm hover:bg-indigo-50 flex items-center justify-between transition-colors">
                      <span className="flex items-center"><Award size={16} className="mr-2 text-indigo-600" /> Status</span>
                      {sortBy === 'status' && (sortBy === 'asc' ? <ArrowUp size={14} className="text-indigo-600" /> : <ArrowDown size={14} className="text-indigo-600" />)}
                    </button>
                  </div>
                )}
              </div>
